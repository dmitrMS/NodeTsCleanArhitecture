...
task.patch('/task/update', async (req, res) => {
  const token = req.headers[authHeader] as string;
  const { task_id, name,description, status_id, priority_id, begin_date, end_date, executor_id } = req.body;
  const verifyWork = await taskHandler.update(token,task_id, name, description, status_id, priority_id, begin_date, end_date, executor_id);
